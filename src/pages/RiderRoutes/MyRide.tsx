...
export default function MyRide() {
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      if (ride?.pickupLocation && ride?.destination) {
        try {
          const coords = `${ride.pickupLocation.coordinates[0]},${ride.pickupLocation.coordinates[1]};${ride.destination.coordinates[0]},${ride.destination.coordinates[1]}`;
  ...
  if (!ride) return <p>No ride data found.</p>;
    ...
    <section className="container mx-auto max-w-4xl mt-20 p-4">
      <div className=" shadow-lg overflow-hidden flex flex-col gap-4">
        ...
        <div className="z-10 min-w-[400px] h-[350px] relative">
          <MapContainer center={pickupCoords} zoom={14} className="h-full w-full">
            <TileLayer
        ...
        <div className="w-full p-4 shadow-lg border flex flex-col md:flex-row justify-around">
          ...
          <div className="flex flex-col justify-center items-center">
            <div className="mt-6 space-y-4">
              ...
              <p className="text-sm"><strong className="text-primary">Destination:</strong> {destinationAddress || `${destinationCoords[0]}, ${destinationCoords[1]}`}</p></div>
          </div>
        </div>
      </div>
      <div>
