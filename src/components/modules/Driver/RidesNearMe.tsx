...
export default function RidesNearMe() {
  ...
  const [decodedRides, setDecodedRides] = useState<any[]>([]);
    ...
    const decodeCoordinates = async () => {
      ...
      const decoded = await Promise.all(
        rides.map(async (ride) => {
          const decodePoint = async (lat: number, lon: number) => {
            try {
          ...
          const pickup = await decodePoint(
            ride.pickupLocation.coordinates[1],
            ride.pickupLocation.coordinates[0]
          );
          const destination = await decodePoint(
      ...
      setDecodedRides(decoded);
      setIsDecoding(false);
    };

    if (rides.length) decodeCoordinates();
  }, [rides]);

  const paginatedRides = decodedRides.slice((currentPage - 1) * limit, currentPage * limit);
  const totalPage = Math.ceil(decodedRides.length / limit);
  ...
  return (
    <div className="flex flex-col items-center px-4 text-center gap-6 w-full">
      ...
      {isDecoding ? (
        <p className="mt-8 text-sm md:text-base">Loading rides near you...</p>
                ...
                <TableBody>
                  {paginatedRides.map((ride) => (
                    <TableRow key={ride._id}>
                      <TableCell
                        className="text-left text-[12px] md:text-xs break-words whitespace-normal max-w-full"
                        title={ride.pickup}
                      >
                        {truncateText(ride.pickup, 30)}
                      </TableCell>
                      <TableCell
                        className="text-left text-[12px] md:text-xs break-words whitespace-normal max-w-full"
      ...
      {acceptedRide && (
        <div className="flex flex-col lg:flex-row justify-between gap-6 lg:gap-20">
          ...
          <Button
            className=" font-semibold  bg-red-600 text-white py-2 hover:bg-red-700 rounded-none"
            onClick={() => navigate(`/my-accepted-ride/${acceptedRide._id}`)}
          >
            Cancel The Accepted Ride
          </Button>
        </div>
      )}
    </div>
